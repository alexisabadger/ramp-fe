...
**Solution:**
On line 95 of index.css, under .RampInputSelect--dropdown-container,
remove this line: 
    position: fixed;
...
on line 16 of InputCheckBox/index.tsx
...
**Solution:**
In App.tsx, on line 64, changed:
      onChange={async (newValue) => {
        if (newValue!.id === "" || newValue === null) {
          await loadAllTransactions()
          return
        }
        await loadTransactionsByEmployee(newValue.id)
      }
If the newValue is null or the id is empty, load all transactions. Otherwise, load transactions by employee.
...
**Solution:**
Changed body of getTransactionsPaginated (in requests.ts) to:
  const start = 0
...
**Solution:**
In App.tsx, I created an additional piece of state to track the loading status of the employees request:
...
**Solution:**
On line 17 of App.tsx, I added a bit of Boolean state that toggles on/off (true/false) when the user selects an employee from the dropdown. This state is then altered in App.tsx's loadAllTransactions and loadTransactionsByEmployee functions. If this state is false, the View More button is not rendered.

const [viewMoreStateButton, setViewMoreStateButton] = useState(true)

The the state is tied with a conditional CSS class in the button:
className={viewMoreStateButton ? "RampButton" : "RampButton--disabled"}
...
**Solution:**
In App.tsx, I used useMemo to check if the end of the transactions has been reached. If it has, setViewMoreState is set to false.
  const noMoreTransactions = useMemo(() => {
      if (paginatedTransactions?.endOfTransactions) {
        setViewMoreStateButton(false)
      }
    },[paginatedTransactions, transactionsByEmployee]
  )

To do this, I added an optional data member for PaginatedResponse, which I return in the getTransactionsPaginated function in requests.ts:
...
**Solution:**
In setTransactionApproval/index.tsx, I imported clearCache from the useCustomFetch hook.
  const { fetchWithoutCache, loading, clearCache } = useCustomFetch()
I then added a second line in the same file, in the setTransactionApproval function, that calls clearCache.
  await clearCache()
Now, transaction approval works as expected.
